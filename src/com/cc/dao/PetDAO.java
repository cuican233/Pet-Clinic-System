...
public class PetDAO
{
    ...
    public void update(Pet pet) throws Exception
    {
        ...
        try
        {
            Class.forName("com.mysql.jdbc.Driver");
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/db_ph?useUnicode=true&characterEncoding=UTF-8","root","cuican233");//  协议://域名(ip):端口/资源（数据库名）
            ps = con.prepareStatement("update t_pet set name=?,birthdate=?,photo=?,ownerId=? where id=?");
            ps.setString(1, pet.getName());
            ps.setString(2, pet.getBirthdate());
            ps.setString(3, pet.getPhoto());
            ps.setInt(4, pet.getOwnerId());
    ...
    public Pet getByName(String petName) throws Exception{
        Pet pet=null;
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ...
            Class.forName("com.mysql.jdbc.Driver");
            con=DriverManager.getConnection("jdbc:mysql://localhost:3306/db_ph?useUnicode=true&characterEncoding=UTF-8","root","cuican233");//  协议://域名(ip):端口/资源（数据库名）
            ps=con.prepareStatement("select * from t_pet where name=?");
