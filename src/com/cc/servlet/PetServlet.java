...
public class PetServlet extends HttpServlet
{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        else if("modifyPet".equals(m))
        {
            modifyPet(request, response);
        }
    }
    ...
    {
        //doPost方法会被多个表单调用，查询宠物、增加宠物,这里需根据不同表单传递的标示参数调用不同的方法
        String m = request.getParameter("m");
        if("search".equals(m))
        {
    ...
    //增加宠物的处理逻辑,用于处理newPetAdd.jsp页面的Post提交操作
    private void newPetAdd_doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        Part p = request.getPart("photo");
        String oldname = getFileName(p);
        //这里需要上传文件，就需要得到上传文件的目标路径，这里的文件保存路径不能是任意路径，只能放到当前应用根目录及其子目录下，
        // 才能够通过浏览器访问，如何得到这个当前应用根目录，一般情况下不能写死路径，应该通过代码动态得到路径
        String photo = "photo/default.jpg";
        if(!oldname.equals(""))//如果旧文件名不为空表示用户上传了照片，则需要保存照片，否则可以省略这个步骤
        {
            String type = oldname.substring(oldname.lastIndexOf("."));//xxxx.xx
            ...
            // 路径有了差文件名，如果使用原文件名，可能出现重名文件，若非要使用原文件名，则可以分文件夹或将文件名改为时间毫秒数
            String saveFile = getRuntimePath() + newname;
            photo = "photo/" + newname;
            p.write(saveFile);//上传文件
        }
    ...
    {
        String filename = "";
        String contentDec = part.getHeader("content-disposition");
    ...
    private void modifyPet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try
        {
            int id = Integer.parseInt(request.getParameter("petId"));//得到vetsearch_result.jsp的“修改”链接发送来的参数客户id
            String name = request.getParameter("petName");
            String OwnerName = request.getParameter("petOwnerName");
            Pet pet = new Pet();
            pet.setId(id);
            pet.setName(name);
            ...
            List<User> users = userDAO.getAllCustomer();
            request.setAttribute("users", users);
            request.getRequestDispatcher("/modifyPet.jsp").forward(request, response);
        }
        catch (Exception e)
        {
            request.setAttribute("msg", e.getMessage());
    ...
    private void modify(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            Pet pet2 = petDAO.getByName(request.getParameter("oname"));
            int id = pet2.getId();
            pet.setId(id);
            new PetDAO().update(pet);
            request.setAttribute("msg", "宠物信息修改成功");
            request.getRequestDispatcher("/petSearch.jsp").forward(request, response);
        } catch (Exception e) {
            request.setAttribute("msg", e.getMessage());
            request.getRequestDispatcher("/petSearch.jsp").forward(request, response);
